...
class _MainPageState extends State<MainPage> {
      ...
      setState(() {
        body = ListView.separated(
            itemBuilder: (context, index) {
              return Text(meals[index]['DDSHI_NM']);
            },
  ...
  /*   print(dt.toString().split(' ')[0].replaceAll('-', ''));
        print(dt.toString().split(' ')[3].replaceAll('-', '')); */
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
          onPressed: showCal, child: Icon(Icons.calendar_month_outlined)),
    );
  }
}
